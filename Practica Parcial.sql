...
LEFT JOIN Factura f2 ON 
    f2.fact_cliente = f.fact_cliente AND YEAR(f2.fact_fecha) % 2 <> 0 -- Clientes con compras en años impares
WHERE YEAR(f.fact_fecha) = 2012
  AND f2.fact_cliente IS NULL -- Excluir clientes que hayan comprado en años impares
GROUP BY c.clie_codigo,c.clie_razon_social
